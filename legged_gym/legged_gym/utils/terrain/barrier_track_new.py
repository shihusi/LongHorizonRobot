...
class BarrierTrack:
    ...
    def get_climb_track(self,
        ...
        climb_height_ = climb_height + (
            self.track_kwargs["climb"].get("fake_offset", 0.) \
            if climb_height > 0. \
            else 0.)
        block_info = torch.tensor([
            climb_depth,
    ...
    def get_stepping_obstacle_info(self, positions):
        ...
        obstacle_info = self.track_info_map[
            track_idx_clipped[:, 0],
            track_idx_clipped[:, 1],
            block_idx_clipped.type(torch.long),
        ] # (N, 3)
    ...
    def get_engaging_block_info(self, base_positions, volume_points_offset):
        ...
        engaging_block_info = self.track_info_map[
            ...
            block_idx_selection.type(torch.long),
            1:
        ]
    ...
    def get_climb_penetration_depths(self,
        ...
        climb_over_mask = torch.logical_and(
            positions_in_block[:, 2] > block_infos[:, 2],
            positions_in_block[:, 2] > 0, # to avoid the penetration of virtual obstacle in climb down.
        ) # (n_points,)

        penetrated_mask = torch.logical_and(
            torch.logical_or(in_up_mask, in_down_mask),
            (torch.logical_not(climb_over_mask)),
        )
        if mask_only:
    ...
    def draw_virtual_climb_track(self,
        ...
        climb_depth = block_info[1]
        climb_height = block_info[2]
        geom = gymutil.WireframeBoxGeometry(
            climb_depth if climb_height > 0 else self.track_kwargs["climb"].get("down_forward_length", 0.1),
            self.env_width,
            climb_height,
            pose= None,
            color= (0, 0, 1),
        ...
        pose = gymapi.Transform(gymapi.Vec3(
            climb_depth/2 + block_origin[0],
            block_origin[1],
            climb_height/2 + block_origin[2],
        ), r= None)
        gymutil.draw_lines(
            geom,
            self.gym,
