...
class LeggedRobotField(LeggedRobot):
    ...
    def _init_buffers(self):
        ...
        self.robot_rigid_body_states = self.all_rigid_body_states.view(self.num_envs, -1, 13)[:, :17, :]
    ...
    def _process_dof_props(self, props, env_id):
        ...
        if hasattr(self.cfg.domain_rand, "joint_friction_range"):
            for i in range(12):
                props["friction"][i] = random.uniform(self.cfg.domain_rand.joint_friction_range[0], self.cfg.domain_rand.joint_friction_range[1])
        if hasattr(self.cfg.domain_rand, "joint_damping_range"):
            for i in range(12):
                props["damping"][i] = random.uniform(self.cfg.domain_rand.joint_damping_range[0], self.cfg.domain_rand.joint_damping_range[1])
    ...
    def get_obs_segment_from_components(self, components):
        ...
        if "all_history" in components:
            segments["all_history"] = (700,)
        ...
        if "action_history" in components:
            segments["action_history"] = (72,)
        
        if "clock_input" in components:
            segments["clock_input"] = (4,)

        if "dof_pos_history" in components:
            segments["dof_pos_history"] = (72,)

        if "all_command_obs" in components:
            segments["all_command_obs"] = (72,)

        if "all_history" in components:
            segments["all_history"] = (740,)
    ...
    #     return self.num_obs #self.get_num_obs_from_components(self.cfg.env.obs_components)    # 244 57
    # @num_obs.setter
